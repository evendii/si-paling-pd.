let handler = (m, { conn, usedPrefix, command, text }) => {
  conn.tembak = conn.tembak || { musuh: [], tembak: [] }
   if(/kiri/i.test(text)) {
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, "©LitRHap", [["Tembak", `${usedPrefix}${command} tembak`]], m)
    return conn.sendButton(m.chat, pos, "©LitRHap", [["←", `${usedPrefix}${command} kiri`], ["→", `${usedPrefix}${command} kanan`]], m)
    ...
    if(conn.tembak.musuh.indexOf("🥷") === conn.tembak.tembak.indexOf("🤠")) return conn.sendButton(m.chat, pos, "©LitRHap", [["Tembak", `${usedPrefix}${command} tembak`]], m)
    return conn.sendButton(m.chat, pos, "©Muhammad Restu", [["←", `${usedPrefix}${command} kiri`], ["→", `${usedPrefix}${command} kanan`]], m)
  }
}
handler.help = ['koboy']
handler.tags = ['rpg']
